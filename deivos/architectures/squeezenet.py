...
def squeeze(x, name, filters):
    ...
    Returns
    -------
    squeeze : tensorflow.python.framework.ops.Tensor
...
def expand(x, name, filters):
    ...
    Returns
    -------
    expand : tensorflow.python.framework.ops.Tensor
    ...
    expand = Concatenate(name=name+'_concat')([expand_1x1, expand_3x3])
    return expand
...
def fire_module(x,
    ...
    Returns
    -------
    fire : tensorflow.python.framework.ops.Tensor
    ...
    if bypass is True:
        num_channels_in = K.int_shape(x)[-1]
        num_channels_out = K.int_shape(fire)[-1]
